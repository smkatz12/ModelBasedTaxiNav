...
redofile = 'downsampled_64.h5'
outfile = 'downsampled_64_mb.h5'
...
def get_data_labels(dataset_dir, verbose=True):
    # if verbose:
    #     print("\nReading " + csv_path)
    ...
    for i, fn in enumerate(labels.filename):
        ...
        right_line, left_line = get_ds_lines(right_labels, left_labels)
...
def get_ds_lines(right_labels, left_labels):
    factor_h = 1 / strideh
    factor_w = 1 / stridew
...
if __name__ == "__main__":
    fil = h5py.File(dataset_dir + redofile, 'r')
    X = fil['X_train']
    y = get_data_labels(dataset_dir)
