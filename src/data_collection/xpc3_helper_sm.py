...
def getStartXY():
    ...
    return 35596.13671875, 46022.41015625

def getRunwayHeading():
    """Get runway heading in the local coordinate frame"""
    return 156.1
...
def rotateToHome(x, y):
    """Rotate to the home coordinate frame.
        ...
        and ends at (0, 980 at the end of the runway). Thus, the x-value
    ...
    """
    heading_deg = getRunwayHeading()
    heading = heading_deg * np.pi / 180.0
    rotx = np.cos(heading) * x + np.sin(heading) * y
    roty = -np.sin(heading) * x + np.cos(heading) * y
    return rotx, roty
...
def setHomeState(client, x, y, theta):
    ...
    position (y), and heading error (theta).
        ...
        Args:
            ...
            y: desired downtrack position [0, 980] (meters)
    ...
    client.sendDREF("sim/flightmodel/position/psi", getRunwayHeading() - theta)
...
def reset(client, cteInit=0, heInit=0, dtpInit=0, noBrake=True):
    ...
    # client.sendPOSI([47.196890, -119.33260, 362.14444,
    #                 0.31789625, 0.10021035, 53.7, 1], 0)
    # Fine-tune position
    ...
    heading = getRunwayHeading() - heInit
    client.sendDREF("sim/flightmodel/position/local_vx",
                    5.0*np.sin(heading*np.pi/180.0))
    client.sendDREF("sim/flightmodel/position/local_vz", -
                    5.0*np.cos(heading*np.pi/180.0))

    # Reset fuel levels
    client.sendDREFs(["sim/flightmodel/weight/m_fuel1",
                     "sim/flightmodel/weight/m_fuel2"], [232, 232])
...
def saveState(client, folder, filename='test.csv'):
    ...
    drefs += ["sim/flightmodel/weight/m_fuel1",
